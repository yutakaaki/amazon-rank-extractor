...
export function extractVolume(title) {
  ...
  const patterns = [
    ...
    /\bV\s*([0-9]{1,4})\b/, // V3
    // 以下は巻数がタイトルに直接くっつく形にも対応する(空白必須にしない)。
    // 例)「THE NEW GATE18 (…)」「追放者食堂へようこそ！11 ～…～」
    /(?:^|[^0-9０-９])([0-9０-９]{1,4})\s*[(（]/, // 呪術廻戦 26 (ジャンプコミックス) / GATE18 (
    /(?:^|[^0-9０-９])([0-9０-９]{1,3})\s*[～〜]/, // タイトル 3 ～サブタイトル～ / ようこそ！11 ～
    /(?:^|[^0-9０-９])([0-9０-９]{1,4})\s*$/, // 末尾の裸の巻数
  ];
